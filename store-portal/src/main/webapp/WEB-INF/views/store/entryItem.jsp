...
<body>
	...
	<table id="contentTable" class="table table-condensed alert">
		...
		<c:forEach items="${order.details}" var="detail">
			<tr>
				...
				<td>${detail.num}</td>
				<td><a href="${ctx}/store/entry/item/delete/${order.id}/${item.id}">删除</a></td>
			</tr>
		</c:forEach>
		</tbody>
	</table>
	...
	<div class="form-actions"  style="text-align: right;">
		<form action="${ctx}/store/entry/send/${order.id}" method="get">
			<input id="submit_btn" class="btn btn-primary" type="submit" value="确认并发送"/>&nbsp;
			<input id="cancel_btn" class="btn" type="button" value="返回入库列表"/>
		</form>
	</div>
	...
	<table id="contentTable" class="table table-striped table-condensed">
		...
		<c:forEach items="${items}" var="item">
			<tr>
				...
				<td>${item.weight}</td>
				<td>
					<input type="text" id="${item.id}_num" class="span4" placeholder="请输入商品数量..">
				    <a class="btn" href="javascript:addItem(${item.id},'${item.id}_num');">添加</a>
				</td>
			</tr>
		</c:forEach>
		</tbody>
